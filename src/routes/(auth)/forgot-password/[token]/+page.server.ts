...
export let actions: Actions = {
    async confirmPasswordReset({ locals, request, params }) {
        ...
        if (!newPassword || !newPasswordConfirm) {
            return invalid(400, { missingFields: true, message: "Missing required fields!" });
        }

        if (newPassword.toString() != newPasswordConfirm.toString()) {
            return invalid(400, { mismatch: true, message: "New password doesn't match." });
        }
        ...
        try {
            await locals.pb.collection('users').confirmPasswordReset(
                ...
                newPasswordConfirm.toString(),
            );
            return { passwordReset: true, message: "Password successfully reset." };
        } catch (e: any) {
            if (e.status === 400) {
                return invalid(e.status, { confirmFailed: true, message: "Failed to reset password, please try again later." });
            }
